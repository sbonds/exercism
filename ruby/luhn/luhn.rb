...
class Luhn
  ...
  def double_with_overflow(first_digit, second_digit)
    doubled_digit = (second_digit || 0) * 2
    (doubled_digit < 10 ? doubled_digit : doubled_digit - 9) + first_digit
  end
  ...
  def luhn_sum
    ...
    string_of_numbers.reverse.chars.map(&:to_i).each_slice(2).sum{ |first_digit, second_digit| double_with_overflow(first_digit, second_digit)}
  end
